...
class MonitorServer {
  ...
  init() {
    ...
    options.onSignal = async function() {
      let promises = MonitorServer.convertListToPromises(that.signalHooks)
      ...
      if (that.boundServer) {
        promises.push(promisify(that.boundServer.close.bind(that.boundServer))())
      }
  ...
  bindHttpServer(server) {
    ...
    this.addLivenessCheck(() => {
      if (!this.boundServer.listening) throw new Error('not listening')
    })
