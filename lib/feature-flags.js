...
class FeatureFlags {
  ...
  init() {
    ...
    if (!instanceId) {
      throw new Error('GitLab Feature Flag initiation failed. You need to set FEATURE_FLAGS_INSTANCE_ID')
    }
    const url = process.env.FEATURE_FLAGS_URL
    if (!url) {
      throw new Error('GitLab Feature Flag initiation failed. You need to set FEATURE_FLAGS_URL')
    }
